...
<div class="row g-3">
  ...
  <div class="col-md-4"><label class="form-label">Release (optional)</label><select name="release_id" class="form-select"><option value="">—</option><?php foreach ($releases as $rel): ?><option value="<?php echo (int)$rel->id; ?>" <?php echo ($item && (int)$item->release_id===(int)$rel->id)?'selected':''; ?>><?php echo htmlspecialchars($rel->version . ' — ' . $rel->title); ?></option><?php endforeach; ?></select></div>
  <div class="col-md-4"><label class="form-label">Related task (optional)</label><select name="task_id" class="form-select"><option value="">—</option><?php foreach ($tasks as $t): ?><option value="<?php echo (int)$t->id; ?>" <?php echo ($item && (int)$item->task_id===(int)$t->id)?'selected':''; ?>><?php echo htmlspecialchars($t->title); ?></option><?php endforeach; ?></select></div>
  ...
  <div class="col-md-3"><label class="form-label">Assign to</label><select name="assigned_to" class="form-select"><option value="">Unassigned</option><?php foreach ($members as $m): ?><option value="<?php echo (int)$m->id; ?>" <?php echo ($item && (int)$item->assigned_to===(int)$m->id)?'selected':''; ?>><?php echo htmlspecialchars($m->name); ?></option><?php endforeach; ?></select></div>
  <div class="col-12"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="3"><?php echo $item?htmlspecialchars($item->description):''; ?></textarea></div>
  <div class="col-12"><label class="form-label">Steps to reproduce</label><textarea name="steps_to_reproduce" class="form-control" rows="4"><?php echo $item?htmlspecialchars($item->steps_to_reproduce):''; ?></textarea></div>
</div>
