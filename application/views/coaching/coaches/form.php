...
<div class="mb-3"><label class="form-label">User</label><select name="user_id" class="form-select" required><?php foreach ($users as $u): ?><option value="<?php echo (int)$u->id; ?>" <?php echo ($row && (int)$row->user_id===(int)$u->id)?'selected':''; ?>><?php echo htmlspecialchars($u->name.' — '.$u->email); ?></option><?php endforeach; ?></select></div>
<div class="mb-3"><label class="form-label">Title</label><input type="text" name="title" class="form-control" value="<?php echo $row ? htmlspecialchars($row->title) : ''; ?>"></div>
<div class="mb-3"><label class="form-label">Bio</label><textarea name="bio" class="form-control" rows="3"><?php echo $row ? htmlspecialchars($row->bio) : ''; ?></textarea></div>
