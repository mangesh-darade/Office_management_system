  ...
  <div class="sidebar-inner p-3">
    <nav class="nav flex-column gap-1 sidebar-nav">
      ...
      $settings_group_show = function_exists('has_module_access') && (
        ...
        has_module_access('api_integrations') ||
      ...
      <div class="nav-item" id="settings-group">
        <a id="settings-parent" class="nav-link sidebar-link sidebar-group-parent <?php echo in_array($active, ['settings','permissions','email-settings','db','reminders','calendar-reminders','activity','departments','designations','statuses','shifts','lead-mapping','system-settings','api-integrations','approvals'], true) || ($active==='settings' && in_array($active_sub, ['types','client-versions','leave-types','holidays','attendance-manage','subscription-builder'], true)) || ($active==='settings' && strpos(uri_string(), 'subscription-builder') !== false) ? 'active' : ''; ?>" href="#">
          <span class="sidebar-group-row-inner">
        ...
        <div class="sidebar-submenu" id="settings-submenu">
          <div class="submenu-list">
            ...
            <a class="submenu-link <?php echo ($active==='settings' && $active_sub==='client-versions')?'active':''; ?>" href="<?php echo site_url('settings/client-versions'); ?>"><i class="bi bi-tag me-2"></i>Client Versions</a>
            <?php endif; ?>
            ...
            <a class="submenu-link <?php echo $active==='calendar-reminders'?'active':''; ?>" href="<?php echo site_url('calendar-reminders'); ?>"><i class="bi bi-calendar-event me-2"></i>Google Calendar Reminders</a>
            <?php endif; ?>
      ...
      <script>initSidebarGroup('settings-group','settings-toggle','settings-parent','settings-submenu','sb_settings_open',<?php echo (in_array($active, ['settings','permissions','email-settings','db','reminders','calendar-reminders','activity','departments','designations','statuses','shifts','approvals','lead-mapping','system-settings','api-integrations'], true) || ($active==='settings' && in_array($active_sub, ['types','client-versions','leave-types','holidays','attendance-manage','subscription-builder'], true)) || ($active==='settings' && strpos(uri_string(), 'subscription-builder') !== false))?'true':'false'; ?>);</script>
      <?php endif; ?>
