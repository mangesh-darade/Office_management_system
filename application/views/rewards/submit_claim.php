...
<div class="mb-3"><label class="form-label">Activity</label><select name="rule_code" class="form-select" required><option value="">— Select —</option><?php foreach ($rules as $r): ?><option value="<?php echo htmlspecialchars($r->code); ?>"><?php echo htmlspecialchars($r->name); ?> (<?php echo ((float)$r->points>=0?'+':'').number_format((float)$r->points,0); ?> pts)</option><?php endforeach; ?></select></div>
<div class="mb-3"><label class="form-label">Team member(s)</label><select name="user_ids[]" class="form-select" multiple size="8" required><?php foreach ($users as $u): ?><option value="<?php echo (int)$u->id; ?>"><?php echo htmlspecialchars($u->name); ?></option><?php endforeach; ?></select><div class="form-text">Hold Ctrl/Cmd to select multiple.</div></div>
