...
<h1 class="h5"><?php echo htmlspecialchars($invoice->invoice_no); ?> — <?php echo htmlspecialchars($client->full_name); ?></h1>
...
<?php foreach ($installments as $inst): ?><tr><td><?php echo (int)$inst->installment_no; ?></td><td><?php echo htmlspecialchars($inst->due_date); ?></td><td>₹<?php echo number_format($inst->amount,2); ?></td><td><?php echo htmlspecialchars($inst->status); ?></td><td><?php if ($inst->status==='pending'): ?><a href="<?php echo site_url('coaching-billing/mark-paid/'.$inst->id.'?redirect='.urlencode(current_url())); ?>">Mark paid</a> · <a href="<?php echo site_url('coaching-payments/pay/'.$inst->id); ?>">Pay online</a><?php endif; ?></td></tr><?php endforeach; ?>
</tbody></table>
