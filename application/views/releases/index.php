...
<tr><td><?php echo esc_view($r->version); ?></td><td><?php echo esc_view($r->title); ?></td><td><?php echo esc_view($r->project_name ?: '—'); ?></td><td><span class="badge bg-light text-dark border"><?php echo esc_view($r->status); ?></span></td><td><?php echo esc_view($r->planned_date ?: '—'); ?></td><td><?php if (function_exists('has_module_access') && (has_module_access('releases_edit') || has_module_access('releases'))): ?><a class="btn btn-sm btn-outline-primary" href="<?php echo site_url('releases/edit/'.$r->id); ?>">Edit</a><?php else: ?>—<?php endif; ?></td></tr>
