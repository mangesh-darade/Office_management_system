...
<div class="row g-3 mb-4">
  <div class="col-lg-4">
    <form method="get" class="card shadow-soft"><div class="card-body">
      ...
      <input type="date" name="date" class="form-control mb-2" value="<?php echo htmlspecialchars($date); ?>" onchange="this.form.submit()">
  ...
  <div class="col-lg-8">
        ...
        <div class="card shadow-soft text-center h-100">
          <div class="card-body">
            ...
            <?php elseif ($hasBf): ?>
              ...
              <div class="badge bg-warning text-dark mt-1">Open until <?php echo htmlspecialchars($bfCut); ?></div>
            <?php else: ?>
              <div class="h3 mb-0 text-muted">—</div>
        ...
        <div class="card shadow-soft text-center h-100">
          <div class="card-body">
            ...
            <?php elseif ($hasLu): ?>
              <div class="h3 mb-0 text-muted">—</div>
              <div class="text-muted small">Half tiffin</div>
              <div class="badge bg-warning text-dark mt-1">Open until <?php echo htmlspecialchars($luCut); ?></div>
            <?php else: ?>
              <div class="h3 mb-0 text-muted">—</div>
    ...
    <?php elseif ($cal): ?>
      <div class="alert alert-info mt-2 mb-0">
        <?php if ((int)$cal->has_breakfast): ?>Breakfast<?php echo $cal->breakfast_note ? ': '.htmlspecialchars($cal->breakfast_note) : ''; ?>. <?php endif; ?>
        <?php if ((int)$cal->has_lunch): ?>Lunch<?php echo $cal->lunch_note ? ': '.htmlspecialchars($cal->lunch_note) : ''; ?>.<?php endif; ?>
      </div>
    <?php endif; ?>
  </div>
</div>
    ...
    <table class="table table-sm mb-0 align-middle">
      ...
      <?php foreach ($pending_requests as $req): ?>
        <tr>
          ...
          <td>
            <span class="badge bg-light text-dark border"><?php echo htmlspecialchars(meal_format_request_value($req->meal_type, $req->current_value)); ?></span>
            <i class="bi bi-arrow-right mx-1 text-primary"></i>
            <span class="badge bg-primary"><?php echo htmlspecialchars(meal_format_request_value($req->meal_type, $req->requested_value)); ?></span>
          </td>
          <td class="small text-muted"><?php echo !empty($req->employee_note) ? htmlspecialchars($req->employee_note) : '—'; ?></td>
          <td>
...
<div class="row g-3">
  ...
  <div class="col-lg-7">
    ...
    <?php if (!$anyLocked): ?>
      <tr><td colspan="3" class="text-muted text-center py-3">Orders still open for this date — counts show after breakfast (<?php echo htmlspecialchars($bfCut); ?>) and/or lunch (<?php echo htmlspecialchars($luCut); ?>) cut-off.</td></tr>
    ...
    ?>
      <tr>
        ...
        <td><?php
          ...
          elseif ($bfPending) {
            echo '<span class="text-muted">' . htmlspecialchars(meal_format_request_value('breakfast', $bfPending->current_value)) . '</span>';
            echo ' <i class="bi bi-arrow-right small text-primary"></i> ';
            echo '<strong class="text-primary">' . htmlspecialchars(meal_format_request_value('breakfast', $bfPending->requested_value)) . '</strong>';
            echo ' <span class="badge bg-warning text-dark ms-1">Pending</span>';
          }
          else { echo $bf > 0 ? htmlspecialchars(meal_order_breakfast_display($o)) : '0'; }
        ?></td>
        <td><?php
          if (!$hasLu) { echo '—'; }
          elseif (!$luLocked) { echo '<span class="text-muted" title="Cut-off not reached yet">Open</span>'; }
          elseif ($luPending) {
            echo '<span class="text-muted">' . htmlspecialchars(meal_format_request_value('lunch', $luPending->current_value)) . '</span>';
            echo ' <i class="bi bi-arrow-right small text-primary"></i> ';
            echo '<strong class="text-primary">' . htmlspecialchars(meal_format_request_value('lunch', $luPending->requested_value)) . '</strong>';
            echo ' <span class="badge bg-warning text-dark ms-1">Pending</span>';
          }
          else {
            echo ($ltMain !== '' || $ltAdd !== '') ? htmlspecialchars(meal_order_lunch_display($o)) : '—';
          }
        ?></td>
      </tr>
