...
<div class="col-md-6"><label class="form-label">Company</label><input name="company" class="form-control" value="<?php echo $row ? esc_view($row->company) : ''; ?>"></div>
<?php if (!empty($show_crm_picker)): ?>
<div class="col-md-6"><label class="form-label">Link to CRM client</label><select name="crm_client_id" class="form-select"><option value="">— None —</option><?php foreach ($crm_clients as $c): ?><option value="<?php echo (int) $c->id; ?>" <?php echo ($row && !empty($row->crm_client_id) && (int) $row->crm_client_id === (int) $c->id) ? 'selected' : ''; ?>><?php echo esc_view($c->label); ?></option><?php endforeach; ?></select></div>
<?php endif; ?>
<div class="col-md-6"><label class="form-label">Primary coach</label><select name="primary_coach_id" class="form-select"><option value="">—</option><?php foreach ($coaches as $c): ?><option value="<?php echo (int)$c->id; ?>" <?php echo ($row && (int)$row->primary_coach_id===(int)$c->id)?'selected':''; ?>><?php echo esc_view($c->name); ?></option><?php endforeach; ?></select></div>
...
<div class="col-12"><label class="form-label">Also assign coaches</label><select name="coach_ids[]" class="form-select" multiple size="4"><?php foreach ($coaches as $c): ?><option value="<?php echo (int)$c->id; ?>" <?php echo in_array((int)$c->id, $assigned, true)?'selected':''; ?>><?php echo esc_view($c->name); ?></option><?php endforeach; ?></select></div>
...
<div class="col-12"><label class="form-label">Notes</label><textarea name="notes" class="form-control" rows="3"><?php echo $row ? esc_view($row->notes) : ''; ?></textarea></div>
</div>
