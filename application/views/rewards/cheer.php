...
<div class="mb-3"><label class="form-label">Colleague</label><select name="recipient_id" class="form-select" required><option value="">Select…</option><?php foreach ($users as $u): ?><option value="<?php echo (int)$u->id; ?>"><?php echo esc_view($u->name); ?></option><?php endforeach; ?></select></div>
