...
<?php foreach ($rows as $r): ?><tr><td><?php echo htmlspecialchars($r->title); ?></td><td><?php echo $r->workshop_date ? date('d M Y', strtotime($r->workshop_date)) : '—'; ?></td><td><?php echo htmlspecialchars($r->status); ?></td><td class="small"><?php if ($r->status==='published'): ?><code><?php echo site_url('coaching/workshop-register/'.$r->id); ?></code><?php endif; ?></td></tr><?php endforeach; ?>
</tbody></table></div>
