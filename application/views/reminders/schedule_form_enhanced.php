...
.schedule-form .nav-pills .nav-link { border-radius: 8px; font-weight: 500; }
      ...
      <div class="row g-3 mb-4">
        <div class="col-md-12">
          ...
          <div class="btn-group" role="group">
            <input type="radio" class="btn-check" name="schedule_type" id="weekly" value="weekly" <?php echo (!isset($schedule->schedule_type) || $schedule->schedule_type === 'weekly') ? 'checked' : ''; ?>>
            <label class="btn btn-outline-primary" for="weekly">
              <i class="bi bi-calendar-week me-2"></i>Weekly
            </label>
            
            <input type="radio" class="btn-check" name="schedule_type" id="once" value="once" <?php echo (isset($schedule->schedule_type) && $schedule->schedule_type === 'once') ? 'checked' : ''; ?>>
        ...
        <div class="row g-3">
          <div class="col-md-12">
            ...
            <div class="weekday-selector">
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '1') !== false) ? 'selected' : ''; ?>" data-day="1">Mon</div>
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '2') !== false) ? 'selected' : ''; ?>" data-day="2">Tue</div>
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '3') !== false) ? 'selected' : ''; ?>" data-day="3">Wed</div>
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '4') !== false) ? 'selected' : ''; ?>" data-day="4">Thu</div>
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '5') !== false) ? 'selected' : ''; ?>" data-day="5">Fri</div>
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '6') !== false) ? 'selected' : ''; ?>" data-day="6">Sat</div>
              <div class="weekday-btn <?php echo (isset($schedule->weekdays) && strpos($schedule->weekdays, '0') !== false) ? 'selected' : ''; ?>" data-day="0">Sun</div>
      ...
      <div class="row g-3 mb-4">
        <div class="col-md-12">
          ...
          <div class="preview-section">
            <div class="mb-2">
              <strong>Subject:</strong> <span id="previewSubject"><?php echo htmlspecialchars(isset($schedule->subject) ? $schedule->subject : 'Daily Team Reminder'); ?></span>
            </div>
            <div class="mb-2">
              <strong>Message:</strong>
            </div>
            <div id="previewBody" style="white-space: pre-wrap; font-size: 0.875rem;">
              <?php echo htmlspecialchars(isset($schedule->body) ? $schedule->body : "Hello {name},\n\nThis is your scheduled reminder..."); ?>
            </div>
          </div>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleScheduleOptions() {
        ...
        } else if (onceRadio && onceRadio.checked) {
            weeklyOptions.style.display = 'none';
            onceOptions.style.display = 'block';
        }
    }
    
    if (weeklyRadio) weeklyRadio.addEventListener('change', toggleScheduleOptions);
    if (onceRadio) onceRadio.addEventListener('change', toggleScheduleOptions);
    ...
    function updateWeekdaysInput() {
        const selected = Array.from(weekdayBtns)
    ...
    function updatePreview() {
        ...
        let subject = subjectInput.value || 'Daily Team Reminder';
